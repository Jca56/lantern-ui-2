//! The 2D draw list: what `lntrn-ui` emits and `Pass2d` draws. Rects,
//! strokes, lines and glyphs live here; circles, arcs, curves, two-axis
//! gradients and pictures in [`crate::shapes`].
//!
...
pub(crate) const MODE_FILL: f32 = 0.0;
pub(crate) const MODE_GLYPH: f32 = 1.0;
pub(crate) const MODE_STROKE: f32 = 2.0;
pub(crate) const MODE_PLAIN: f32 = 3.0;
pub(crate) const MODE_SHADOW: f32 = 4.0;
pub(crate) const MODE_IMAGE: f32 = 5.0;
...
impl DrawList {
    ...
    pub(crate) fn push_quad(&mut self, corners: [Vec2; 4], uvs: [[f32; 2]; 4], color: [f32; 4], rect: [f32; 4], params: [f32; 4]) {
        self.push_quad_colors(corners, uvs, [color; 4], rect, params);
    }

    /// Corners are TL, TR, BR, BL, each with its own color (linear).
    pub(crate) fn push_quad_colors(&mut self, corners: [Vec2; 4], uvs: [[f32; 2]; 4], colors: [[f32; 4]; 4], rect: [f32; 4], params: [f32; 4]) {
        let clip = self.clip4();
