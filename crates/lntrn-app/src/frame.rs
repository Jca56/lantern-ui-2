//! What both harnesses share: the GPU-side objects of an app and of each
//! of its windows, the rebuild loop, and drawing one frame into a
//! swapchain.
...
/// What every window of an app shares on the GPU: the device, and the
/// pictures the host uploaded (a handle draws in any window).
...
impl Gfx {
    ...
    pub fn resize(&mut self, shared: &GpuShared, width: u32, height: u32) {
        self.surface.resize(&shared.gpu, width, height);
        self.pool.trim();
    }
...
pub(crate) fn rebuild<H: AppHost>(shared: &mut GpuShared, gfx: &Gfx, host: &mut H, shell: &mut Shell<H>, text: &mut TextEngine, draw: &mut DrawList, events: &[Event], scale: f64, ws: WindowState, clipboard: &mut Clipboard) -> (ShellOutput, bool) {
    ...
    for _ in 0..MAX_REBUILDS {
        ...
        // click.
        if host.after_rebuild(&shared.gpu, &mut shared.images, shell) {
            again = true;
        }
        out = Some(o);
        if !again {
...
/// Draw the list into window `window`'s swapchain and present. `false`
/// when no frame could be acquired (the caller should try again soon).
/// `before_present` runs between the queue submit and the present
/// (winit's notify).
#[allow(clippy::too_many_arguments)]
pub(crate) fn draw_frame<H: AppHost>(shared: &GpuShared, gfx: &mut Gfx, host: &mut H, text: &mut TextEngine, draw: &DrawList, clear: lntrn_math::Color, window: u32, before_present: impl FnOnce()) -> bool {
    let size = gfx.surface.size();
    let Some(frame) = gfx.surface.acquire(&shared.gpu) else {
        return false;
    };
    let view = frame.texture.create_view(&wgpu::TextureViewDescriptor::default());
    let mut encoder = shared.gpu.create_encoder("lantern frame");

    let (pass2d, images) = (&mut gfx.pass2d, &shared.images);
    ...
    host.render(&mut RenderCx { gpu: &shared.gpu, graph: &mut graph, backbuffer, depth, size, window });
    ...
    graph.execute(&shared.gpu, &mut gfx.pool, &mut encoder);
    gfx.pool.end_frame();

    shared.gpu.queue.submit([encoder.finish()]);
    before_present();
    frame.present();
