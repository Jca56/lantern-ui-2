...
struct Shared {
    ...
    /// An offer a drag brought in; we take no drops, it goes on leave.
    dnd: Option<*mut Proxy>,
...
unsafe extern "C" fn registry_global(data: *mut c_void, registry: *mut Proxy, name: u32, interface: *const c_char, _version: u32) {
    ...
    unsafe {
        ...
        if iface == c"wl_seat" && shared.seat.is_null() {
            shared.seat = bind(&shared.lib, registry, name, &ffi::WL_SEAT);
        } else if iface == c"wl_data_device_manager" && shared.manager.is_null() {
            shared.manager = bind(&shared.lib, registry, name, &ffi::WL_DATA_DEVICE_MANAGER);
        }
    }
}
...
unsafe fn bind(lib: &Lib, registry: *mut Proxy, name: u32, interface: &'static Interface) -> *mut Proxy {
    // SAFETY: the signature is `usun`: name, interface name, version, new id.
    unsafe { (lib.marshal)(registry, op::REGISTRY_BIND, interface, 1, 0, name, interface.name, 1u32, null_mut::<Proxy>()) }
}
...
unsafe extern "C" fn dev_enter(data: *mut c_void, _: *mut Proxy, _serial: u32, _surface: *mut Proxy, _x: i32, _y: i32, offer: *mut Proxy) {
    unsafe { (*(data as *mut Shared)).dnd = (!offer.is_null()).then_some(offer) };
}
unsafe extern "C" fn dev_leave(data: *mut c_void, _: *mut Proxy) {
    unsafe {
        let shared = &mut *(data as *mut Shared);
        if let Some(offer) = shared.dnd.take() {
            destroy_offer(shared, offer);
        }
    }
}
unsafe extern "C" fn dev_motion(_: *mut c_void, _: *mut Proxy, _: u32, _: i32, _: i32) {}
unsafe extern "C" fn dev_drop(_: *mut c_void, _: *mut Proxy) {}
...
unsafe fn destroy_offer(shared: &mut Shared, offer: *mut Proxy) {
    ...
    unsafe { (shared.lib.marshal)(offer, op::OFFER_DESTROY, null::<Interface>(), 1, ffi::MARSHAL_FLAG_DESTROY) };
}
...
unsafe extern "C" fn source_cancelled(data: *mut c_void, source: *mut Proxy) {
    ...
    unsafe {
        ...
        (shared.lib.marshal)(source, op::SOURCE_DESTROY, null::<Interface>(), 1, ffi::MARSHAL_FLAG_DESTROY);
        drop(src);
    }
}
...
impl Clipboard {
    ...
    pub unsafe fn new(display: *mut c_void) -> Option<Clipboard> {
        ...
        unsafe {
            ...
            let shared = Box::into_raw(Box::new(Shared { lib, display, queue, seat: null_mut(), manager: null_mut(), serial: 0, offers: HashMap::new(), selection: None, dnd: None, source: None, owned_text: None }));
            ...
            me.device = (lib.marshal)(manager, op::DDM_GET_DATA_DEVICE, &ffi::WL_DATA_DEVICE, 1, 0, null_mut::<Proxy>(), seat);
    ...
    pub fn read(&mut self) -> Option<String> {
        ...
        unsafe {
            (shared.lib.marshal)(offer, op::OFFER_RECEIVE, null::<Interface>(), 1, 0, mime.as_ptr(), theirs.as_raw_fd() as c_int);
    ...
    pub fn write(&mut self, text: &str) -> bool {
        ...
        unsafe {
            ...
            if let Some((old, data)) = shared.source.take() {
                (shared.lib.marshal)(old, op::SOURCE_DESTROY, null::<Interface>(), 1, ffi::MARSHAL_FLAG_DESTROY);
                drop(Box::from_raw(data));
            }
            let source = (shared.lib.marshal)(shared.manager, op::DDM_CREATE_DATA_SOURCE, &ffi::WL_DATA_SOURCE, 1, 0, null_mut::<Proxy>());
            if source.is_null() {
                return false;
            }
            ...
            for mime in MIMES {
                (shared.lib.marshal)(source, op::SOURCE_OFFER, null::<Interface>(), 1, 0, mime.as_ptr());
            }
            (shared.lib.marshal)(self.device, op::DEVICE_SET_SELECTION, null::<Interface>(), 1, 0, source, shared.serial);
            (shared.lib.flush)(shared.display);
    ...
    fn drop(&mut self) {
        ...
        unsafe {
            ...
            if let Some((source, data)) = shared.source {
                (lib.marshal)(source, op::SOURCE_DESTROY, null::<Interface>(), 1, ffi::MARSHAL_FLAG_DESTROY);
                drop(Box::from_raw(data));
            }
            for offer in shared.offers.keys() {
                (lib.marshal)(*offer as *mut Proxy, op::OFFER_DESTROY, null::<Interface>(), 1, ffi::MARSHAL_FLAG_DESTROY);
            }
